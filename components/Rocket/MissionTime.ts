...
/** Booster engine plumes begin fade when mission clock passes T+0:25 */
export const BOOSTER_FADE_AT_MISSION_SEC = 20;
...
/**
 * T+0:40 — main-stage split: top stack stays put; lower first stage falls and the whole
 * lower body (tank, octoweb, plumes) fades out in this wall time.
 */
export const CORE_STAGING_AT_MISSION_SEC = 40;

/** Wall time for lower-stage fall (ease) and 1 → 0 fade (plumes + wireframe). */
export const CORE_BOTTOM_STAGING_WALL_MS = 8000;

/**
 * T+0:41.5 — second-stage (single) engine plume: opacity 0 → 1 (mission clock), after main
 * stack separation (T+0:41.5 = 41.5 s in `getMissionSec` time base).
 */
export const S2_PLUME_AT_MISSION_SEC = CORE_STAGING_AT_MISSION_SEC + 1.5;
 ...
 * T+0:45 — upper stack exits; T+0:50 — full reset + fade-in from T+0.
 */
export const LOOP_PERIOD_MISSION_SEC = 50; // T+0:50 (rocket loop reset)

/** Upper stack (S2 + fairing) rises + fades out from this mission second (T+0:45). */
export const LOOP_UPPER_EXIT_START_MISSION_SEC = 45; // T+0:45

/** Fade entire rocket in over this many mission seconds after each loop reset (from T+0). */
export const LOOP_FADE_IN_MISSION_SEC = 2.5;
