...
export default function useTypewriter<T extends HTMLElement>(text: string, delay: number, startDelay: number) {
    const typewriterRef = useRef<T>(null);

    const typewrite = useCallback((txt: string, ref: RefObject<T>) => {
        if (!ref.current || !txt) {
            return;
        }
