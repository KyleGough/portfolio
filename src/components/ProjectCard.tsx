...
export default function ProjectCard(props: { src: string, alt: string, title: string, tagline: string, date: string, link: string, chipText: string }) {
    return (
        <Link to={props.link} className='rounded-2xl overflow-hidden border-2 border-white shadow drop-shadow-lg group'>
            <img className='w-full' src={props.src} alt={props.alt} />
            <div className='text-white p-4 font-bold group-hover:brightness-125 group-focus:brightness-125 bg-nav-light transition-all duration-100'>
